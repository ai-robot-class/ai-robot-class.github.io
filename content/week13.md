...
> 同样的代码框架，本周作业（13.7.4）和第 14 周机器狗方向小组项目都会用到。
...
根据你的兴趣方向（或第 14 周小组项目后续拓展），推荐三条学习路径：
...
- 四足机器人结构、步态演进史与 PyBullet 仿真入门
- 用 AI 辅助调试，让机器狗在仿真中站住并向前行走

### 必做作业（第 13 周）

见 **13.7.4**，在仓库 `week13_walk/` 目录提交：

| 文件 | 说明 |
|------|------|
| `quadruped_walk.py` | 调通后的最终代码 |
| `ai_chat_log.md` | 与 AI 的完整对话记录（≥5 轮） |
| `reflection.md` | 反思报告（≥300 字，回答讲义中的 3 个问题） |

### 选修拓展

- **13.7.5**：强化学习（PPO）训练机器狗（可选）
- **13.8**：MATRiX 工业级仿真平台（课堂演示 / 有显卡同学选修）

### 与第 14 周的关系

> **期末小组项目不在本周提交。**  
> 第 14 周才是期末项目实施与交付周（手机遥控 + 迷宫探索 + PDF 报告书），请阅读 [第 14 周讲义](week14.md)。
...
*第 13 周结束。下一周进入期末项目实施（第 14 周小组项目）。*
